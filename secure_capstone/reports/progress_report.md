...
**메타데이터 보정 (meta_adjust)**

등록된 패키지의 소스 점수에 메타데이터 기반 보정을 적용합니다.
각 속성은 **한 번만 평가**되며, 의심 가산과 신뢰 감산이 중복되지 않습니다.
...
| | ≥ 10개 | -5 | 신뢰 (활성) |
| | ≥ 50개 | -10 | 신뢰 (고활성) |
| 유사 이름 | 편집거리 ≤2 | +10 | 의심 (독립 시그널) |

```
최종 점수 = max(소스 점수 + meta_adjust, 0)
```

예시:
```
sklearn: 소스 10점 + 유사이름 +10 + 등록 3925일 -10 + 버전 10개 -5 = 5점 (LOW)
flask:   소스 0점 + 등록 5841일 -10 + 버전 64개 -10 = 0점 (LOW)
신규 악성: 소스 40점 + 등록 5일 +10 + 버전 1개 +5 = 55점 (MEDIUM)
```
