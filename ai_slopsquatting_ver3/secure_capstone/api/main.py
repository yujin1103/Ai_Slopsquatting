...
async def _analyse_package(
            ...
            if all_files:
                ...
                for f in all_files:
                    if "upload_time" not in f:
                        continue
                    raw = f["upload_time"].replace("Z", "+00:00")
    ...
    if not pypi_exists and not npm_exists:
        ...
        # 신뢰도 할인: 오래되고 활성화된 패키지는 소스 점수 감쇄
        trust_factor = 1.0
        ...
        adjusted_source = int(source_score_raw * trust_factor)
        final_score = min(adjusted_source + meta_suspicion, 100)
